...
class ASTGCN(BaseModel):
    def __init__(self,
        ...
        self.BlockList = nn.ModuleList([ASTGCN_block(self.device, self.input_dim, K, nb_chev_filter,
                                       nb_time_filter, time_strides, cheb_polynomials, self.num_nodes, self.seq_len)])

        self.BlockList.extend([ASTGCN_block(self.device, nb_time_filter, K, nb_chev_filter, nb_time_filter,
                              1, cheb_polynomials, self.num_nodes, self.seq_len//time_strides) for _ in range(nb_block-1)])
