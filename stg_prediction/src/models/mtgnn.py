...
class MTGNN(BaseModel):
    def __init__(self, 
        ...
        for i in range(1):
            ...
            for j in range(1,layers+1):
                ...
                # if self.seq_length>self.receptive_field:
                #     self.norm.append(LayerNorm((residual_channels, self.num_nodes, self.seq_length - rf_size_j + 1),elementwise_affine=layer_norm_affline))
                # else:
                #     self.norm.append(LayerNorm((residual_channels, self.num_nodes, self.receptive_field - rf_size_j + 1),elementwise_affine=layer_norm_affline))
    ...
    def forward(self, input, idx=None):
        ...
        for i in range(self.layers):
            ...
            # if idx is None:
            #     x = self.norm[i](x,self.idx) # a normalization layer
            # else:
            #     x = self.norm[i](x,idx)
