...
class M2G4RTP(nn.Module):
    ...
    def forward(self, V, V_reach_mask, start_fea, cou_fea, aoi_feature_steps, aoi_start_steps, aoi_pos_steps, aoi_len_steps, aoi_index_steps, E, is_train):
        ...
        aoi_represent = self.input_layer_aoi(aoi_feature_steps.float())  # [B*T, N, H]
        ...
        if is_train:
            ...
            aoi_index_order = aoi_index_steps.unsqueeze(-1).expand(b, seq, aoi_real_sort_info.shape[-1]).long()  # aoi_index: 排序后真实到达aoi顺序
